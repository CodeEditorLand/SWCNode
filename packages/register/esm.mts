...
const addShortCircuitSignal = <T extends ResolveFnOutput | LoadFnOutput>(
	input: T,
): T => {
	return {
		...input,
...
export const resolve: ResolveHook = async (specifier, context, nextResolve) => {
	...
	if (!context.parentURL || parsedUrl.protocol === "file:") {
		...
		if (ext === ".js") {
			format =
				(await getPackageType(specifier)) === "module"
					? "module"
					: "commonjs";
		} else {
			format =
				EXTENSION_MODULE_MAP[ext as keyof typeof EXTENSION_MODULE_MAP];
		}
	...
	if (error) {
		throw new Error(
			`${error}: ${specifier} cannot be resolved in ${context.parentURL}`,
		);
	}
